...
import { Plus, Search, Edit2, Trash2, Eye, Pin } from "lucide-react";
...
import { mockPosts, navSections, type Post } from "@/data/mockData";
...
const toAdminPost = (p: Post): AdminPost => ({
  ...p,
  status: "published",
  body: "",
});
...
export default function AdminPosts() {
  const [posts, setPosts] = useState<AdminPost[]>(mockPosts.map(toAdminPost));
  ...
  const handleSave = () => {
    if (editingPost) {
      setPosts((prev) =>
        prev.map((p) => (p.id === editingPost.id ? { ...form, id: editingPost.id } as AdminPost : p))
      );
    } else {
      setPosts((prev) => [
        { ...form, id: Date.now().toString() } as AdminPost,
        ...prev,
      ]);
    }
    setDialogOpen(false);
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  value={form.section}
                  onValueChange={(v) => setForm({ ...form, section: v, category: "" })}
                >
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={!form.title || !form.section}>
