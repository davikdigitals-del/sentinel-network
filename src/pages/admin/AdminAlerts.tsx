...
export default function AdminAlerts() {
  ...
  const handleSave = () => {
    if (editing) {
      setAlerts((prev) =>
        prev.map((a) =>
          a.id === editing.id ? { ...a, text, priority } : a
        )
      );
    } else {
      setAlerts((prev) => [
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-destructive/30">
        ...
        <CardContent>
          <div className="bg-destructive text-destructive-foreground rounded-sm p-3 flex items-center gap-3">
      ...
      <Card>
        <CardContent className="p-0 divide-y divide-border">
          {alerts.length === 0 && (
            <p className="p-8 text-center text-muted-foreground">No alerts configured.</p>
          )}
          {alerts.map((alert) => (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Alert" : "Create Alert"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="ALERT: Description of the emergency..."
              />
            </div>
            <div>
